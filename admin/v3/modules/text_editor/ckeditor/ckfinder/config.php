...
//$baseUrl = 'http://'.$_SESSION["userhost"].'/'; 

//$baseUrl = str_replace('www/','','/'.$_SESSION["ftppasta"]).'/';

//$baseUrl = $_SESSION["diradmin"].'/';
...
$folder  = implode('/', $folder).'/images/content/';                        
$folder .= $_SESSION['usernivel'] > 1 ? 'user_'.$_SESSION['usernivel'].'/' : '';

$baseDir = $_SERVER['DOCUMENT_ROOT'].$folder;
$baseUrl = 'http://'.$_SERVER['SERVER_NAME'].$folder;


//$fp = fopen('infockfinder.txt','w+');fputs($fp, $baseUrl.' - '.$_SESSION['username']);fclose($fp);
