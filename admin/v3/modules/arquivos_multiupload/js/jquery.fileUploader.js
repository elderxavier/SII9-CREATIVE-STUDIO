...
(function($) {
	...
	$.fn.fileUploader = function(config){
			...
			$(pxUploadForm).append(
				...
				.append(' \
					<ul class="actions ui-helper-clearfix"> \
						<li title="Enviar" class="upload ui-state-default ui-corner-all"> \
							<span class="ui-icon ui-icon-circle-triangle-e"></span> \
						</li> \
						<li title="Remover" class="delete ui-state-default ui-corner-all"> \
		...
		*/
		function afterEachUpload(formId, data, status, errorThrown) {
			...
			if (status == 'success'){
				...
				$uploadData.find('.cancel').removeClass('cancel').addClass('success').attr('title', 'Enviado');
				
			} else if (status == 'error'){
				
				$uploadData.removeClass('uploading').addClass('error');
				$uploadData.find('.cancel').removeClass('cancel').addClass('error').attr('title', 'Erro!');
				...
				$uploadData.find('.cancel').removeClass('cancel').addClass('canceled').attr('title', 'Cancelado');
				$uploadData.children('.status').html( 'Cancelado' );
			}
			...
			$uploadData.find('.upload').fadeOut();
			
			//$uploadData.find('.actions').html('<li class="success"><span>OK</span></li>');
			//$uploadData.fadeOut();
		...
		*/
		function toogleCancel(cancel) {
			...
			if (cancel) {
				...
				//Cancel Button
				$(config.buttonClear, pxButton).attr({ id: 'px-cancel', title: 'Cancelar' });
			} else {
				//Clear button
				$('#px-cancel', pxButton).attr({ id: buttonClearId, title: 'Limpar' });
			}
		}
		...
		*/
		...
		$('.delete, .success, .error', pxUploadForm).live('click', function(){
		...
		*/
		$('.cancel', pxUploadForm).live('click', function() {
			...
			if (!isHtml5) {
				$form = $(this).parents('.upload-data').children('form');
				$form.remove();
				afterEachUpload($form.attr('id'), null, 'abort', 'Cancelado');
			}
		});
		...
		*/
		$('#px-cancel', pxButton).live('click', function(){
			...
			$('form', pxUploadForm).each(function(){
				afterEachUpload($(this).attr('id'), null, 'abort', 'Cancelado');
			});
