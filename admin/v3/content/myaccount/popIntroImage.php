...
<script type="text/javascript">
	...
	function finishUploadIntroImage(data){	
		...
		if (data){		
			$('#popIntroImage #thumbPerfil, #myaccount #myImageHeadding').attr('src', data).load();
	...
	function removeIntroImage(){	
		...
		if(!removeIntroImageLocked){		
			...
			$.ajax({
				type: 'post',
				data: {'act': 'Remove Image Intro'},
				url: _fullpath +"/data/Myaccount.php",
				timeout: 3000,
				success: function(data){
					if(data == true){
						$('#popIntroImage #thumbPerfil, #myaccount #myImageHeadding').attr('src', '<?=_fullpath?>/images/no-image.png').load();
					}
					else {
	...
	//------------------------------------------------------------------------------------------
	$(function(){		
		$("#myaccount #myImageHeadding").click(openPopIntroImage);
	...
	<div id="popIntroImage" class="uiPopUp half addIntroImage draggable" style="display:none">
		...
		<div class="uiPopContent">		
			<div class="uiPopHolder">
				...
				<form id="formIntroImage" action="<?=_fullpath?>/data/Myaccount.php" target="upload_target" method="post" enctype="multipart/form-data">
				<!-- <form id="formIntroImage" action="<?=_fullpath?>/data/Myaccount.php" method="post" enctype="multipart/form-data">-->
