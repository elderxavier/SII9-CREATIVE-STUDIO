
<?php defined( 'NWD_ACCESS' ) or die( 'Acesso restrito' ); ?>


<script type="text/javascript">

	//------------------------------------------------------------------------------------------
	// DELETE CONFIRM
	//------------------------------------------------------------------------------------------
	function openDeleteMenuConfirm(id){
		$("#popDeleteMenuConfirm").popUpShow();
		
		$("#popDeleteMenuConfirm #cancelBtn").click(closeDeleteMenuConfirm);
		$("#popDeleteMenuConfirm #confirmBtn").click(function(){
			deleteMenu(id);
			closeDeleteMenuConfirm();
		});
	}
	
	
	//------------------------------------------------------------------------------------------
	// EXCLUIR ALBUNS
	//------------------------------------------------------------------------------------------
	var excluirLocked = false;
	
	function deleteMenu(id){	
		
		if(!excluirLocked){
		
			$.ajax({
				type: 'post',
				data: {
					'act': 'Excluir',
					'id': id
				},
				url: "<?=_fullpath?>/data/MenuConfig.php",
				timeout: 3000,
				success: function(data){
					if(data == true){
						$('#list-menu #item'+id).fadeOut(function(){$(this).remove()});
					}
					else {
						alert(data);
					}
					
					excluirLocked = false;			
				},
				error: function(){
					excluirLocked = false;	
				}
			})		
		}		
	}
	
	//------------------------------------------------------------------------------------------
	// CLOSE POP UP
	//------------------------------------------------------------------------------------------
	function closeDeleteMenuConfirm(){
		$("#popDeleteMenuConfirm").popUpHide();		
	}
</script>

<div id="popDeleteMenuConfirm" class="uiPopUp half uiAlert" style="display:none">
	<div class="uiPopContent">	
		<h1 id="titulo" class="uiBoxTitle">Deseja excluir este item?</h1>		
		<div class="uiPopHolder">	
			Os dados apagados não poderão mais ser recuperados.
		</div>
		<div class="uiBoxBottom">
			<button id="cancelBtn" class="uiButton" >Cancelar</button>
			<button id="confirmBtn" class="uiButtonConfirm" >Confirmar</button>
		</div>
	</div>
</div>