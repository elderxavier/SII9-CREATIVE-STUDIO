...
if(isset($_GET[GET_TRACK_OUTPUT_TYPE]) && ($_GET[GET_TRACK_OUTPUT_TYPE] == "jscript" || $_GET[GET_TRACK_OUTPUT_TYPE] == "jcrpt"))
{
	...
	savePassThruToCookie($fullname,$email,$company,$question,$phone,$EXTERNALUSER->Customs);
    ...
    $TRACKINGSCRIPT = str_replace("<!--get_track_params-->",getParams("",array("ws"=>true,"ecsgs"=>true,"ecsge"=>true,"ecsc"=>true,"ecsy"=>true,"ecsx"=>true,"ecsb"=>true,"ecsa"=>true,"ecslw"=>true,"echc"=>true,"ecfs"=>true,"ecfe"=>true,"echt"=>true,"echst"=>true,"ecoht"=>true,"ecohst"=>true,"ovlto"=>true,"ovlt"=>true,"ovlp"=>true,"ovlml"=>true,"ovlmr"=>true,"ovlmt"=>true,"ovlmb"=>true,"ovls"=>true,"ovloo"=>true,"ovlc"=>true,"ovlapo"=>true,"ovlct"=>true,"intgroup"=>true,"intid"=>true,"pref"=>true,"cboo"=>true,"hg"=>true,"fbpos"=>false,"fbw"=>false,"fbh"=>false,"fbshx"=>false,"fbshy"=>false,"fbshb"=>false,"fbshc"=>false,"fbmt"=>false,"fbmr"=>false,"fbmb"=>false,"fbml"=>false,"fboo"=>false,"eca"=>true,"ecw"=>true,"ech"=>true,"ecmb"=>true,"ecml"=>true)),$TRACKINGSCRIPT);
    $TRACKINGSCRIPT = str_replace("<!--server-->",LIVEZILLA_URL,$TRACKINGSCRIPT);
}
else
{
	...
	define("IS_FLOOD",$BROWSER->GetFirstCall() && isFlood(getIP(),CALLER_USER_ID));

    if(IS_FILTERED || IS_FLOOD)
    {
	...
	if(!empty($_GET[GET_EXTERN_USER_NAME]))
	...
	if(!empty($_GET[GET_EXTERN_USER_EMAIL]))
	...
	if(!empty($_GET[GET_EXTERN_USER_COMPANY]))
	...
	if(!empty($_GET[GET_EXTERN_USER_QUESTION]))
	...
	if(JAVASCRIPT)
	{
		if(isset($_GET[GET_TRACK_RESOLUTION_WIDTH]))
		{
			...
			$currentURL = new HistoryURL(getOParam(GET_TRACK_URL,"",$nu,FILTER_SANITIZE_URL,null,500),getOParam(GET_TRACK_SPECIAL_AREA_CODE,"",$nu,null,null,255),getOParam(GET_EXTERN_DOCUMENT_TITLE,"",$nu,null,null,255),getOParam(GET_TRACK_REFERRER,"",$nu,FILTER_SANITIZE_URL,null,510),time());
