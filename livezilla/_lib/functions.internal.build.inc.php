...
function demandEmails($xml="",$count=0,$lmc=0)
{
    ...
    if($count>0)
    {
        ...
        $result = queryDB(true,"SELECT MAX(`created`) AS `lmc` FROM `".DB_PREFIX.DATABASE_TICKET_EMAILS."` WHERE `group_id` IN ('".implode("','",$INTERNAL[CALLER_SYSTEM_ID]->Groups)."') ;");
        if($result && $row = DBManager::FetchArray($result))
            $lmc = $row["lmc"];
    }
    $RESPONSE->Messages .= "<de dut=\"".base64_encode($DUT[DATA_UPDATE_KEY_EMAILS])."\" lmc=\"".base64_encode($lmc)."\" c=\"".base64_encode($count)."\">\r\n" . $xml . "\r\n</de>";
}
...
function demandTickets($xml="",$q_filter="",$q_searchw="",$q_searchf="",$c_total=0,$c_totalread=0,$c_totalquery=0,$c_lmc=0)
{
    global $RESPONSE,$INTERNAL,$DUT;
    $permission = $INTERNAL[CALLER_SYSTEM_ID]->GetPermission(PERMISSION_MESSAGES);
    if($permission != PERMISSION_NONE)
    {
        ...
        $result = queryDB(true,"SELECT MAX(`t2`.`created`) AS `lmc` FROM `".DB_PREFIX.DATABASE_TICKETS."` AS `t1` INNER JOIN `".DB_PREFIX.DATABASE_TICKET_MESSAGES."` AS `t2` ON `t1`.`id`=`t2`.`ticket_id` WHERE ".$q_grperm."(`t2`.`type`=0 OR `t2`.`type`=3);");
        if($result && $row = DBManager::FetchArray($result))
            $c_lmc = $row["lmc"];
    }
    $RESPONSE->Messages .= "<dt dut=\"".base64_encode($DUT[DATA_UPDATE_KEY_TICKETS])."\" lmc=\"".base64_encode($c_lmc)."\" p=\"".base64_encode($loads)."\" t=\"".base64_encode($c_total)."\" r=\"".base64_encode($c_totalread)."\" q=\"".base64_encode($c_totalquery)."\">\r\n" . $xml . "\r\n</dt>";
}
