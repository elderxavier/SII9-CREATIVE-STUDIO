...
class Chat extends BaseObject
{
    ...
    function Generate($_chatId,$_externalFullname,$_plain=false,$_html=false,$_question="",$_startTime=0, $firstpost="")
    {
        ...
        $result_posts = queryDB(true,"SELECT `sender_name`,`text`,`sender`,`time`,`micro`,`translation` FROM `".DB_PREFIX.DATABASE_POSTS."` WHERE `repost`=0 AND `receiver`=`receiver_original` AND `chat_id` = '". DBManager::RealEscape($_chatId)."' GROUP BY `id` ORDER BY `time` ASC, `micro` ASC;");
        ...
        $result_files = queryDB(true,"SELECT `created`,`file_name`,`file_id`,`permission`,`download` FROM `".DB_PREFIX.DATABASE_CHAT_FILES."` WHERE `chat_id` = '". DBManager::RealEscape($_chatId)."' ORDER BY `created` ASC;");
        ...
        $result_forwards = queryDB(true,"SELECT `initiator_operator_id`,`invite`,`target_group_id`,`target_operator_id`,`created` FROM `".DB_PREFIX.DATABASE_CHAT_FORWARDS."` WHERE `auto`=0 AND `invite`=0 AND `chat_id` = '". DBManager::RealEscape($_chatId)."' ORDER BY `created` ASC;");
...
class HistoryUrl
{
	...
	function Save($_browserId,$_entrance)
	{
		...
		queryDB(true,"INSERT IGNORE INTO `".DB_PREFIX.DATABASE_VISITOR_BROWSER_URLS."` (`browser_id`, `entrance`, `referrer`, `url`, `params`, `untouched`, `title`, `ref_untouched`, `is_entrance`, `is_exit`) VALUES ('".DBManager::RealEscape($_browserId)."', '".DBManager::RealEscape($this->Entrance)."', '".DBManager::RealEscape($this->Referrer->Save())."', '".DBManager::RealEscape($this->Url->Save())."', '".DBManager::RealEscape($this->Url->Params)."', '".DBManager::RealEscape($this->Url->Untouched)."', '".DBManager::RealEscape($this->Url->PageTitle)."', '".DBManager::RealEscape($this->Referrer->Untouched)."', ".DBManager::RealEscape($_entrance ? 1 : 0).", 1);");
	}
}
...
class BaseURL
{
	...
	function Save()
	{
		...
		queryDB(true,"INSERT IGNORE INTO `".DB_PREFIX.DATABASE_VISITOR_DATA_PAGES."` (`id`, `path`, `domain`,  `title`, `area_code`) VALUES (NULL, '".DBManager::RealEscape($pid)."',  '".DBManager::RealEscape($did)."',  '".DBManager::RealEscape($tid)."', '".DBManager::RealEscape($cid)."');");
...
class TicketMessage extends Action
{
    ...
    function ApplyCustomFromPost($_count,$_customStart,$_change=false,$_ticket=null,$_operatorId="")
    {
        global $INPUTS;
        foreach($INPUTS as $index => $input)
            if($input->Custom && $input->Active)
            {
                ...
                if($_change && $compare != $_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $_count . "_vd_" . $_customStart])
                    $this->ChangeValue($_ticket,$index+16,$_operatorId,$compare,$_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $_count . "_vd_" . $_customStart]);
                $this->Customs[$index] = $_POST[POST_INTERN_PROCESS_TICKET_ACTIONS . "_" . $_count . "_vd_" . $_customStart++];
            }
        return $_customStart;
    }
...
class TicketEmail
{
    ...
    function Save()
    {
        ...
        {
            $time=GetUniqueMessageTime(DATABASE_TICKET_EMAILS,"edited");
            queryDB(true, "REPLACE INTO `" . DB_PREFIX . DATABASE_TICKET_EMAILS . "` (`email_id`, `mailbox_id`, `sender_email`, `sender_name`,`sender_replyto`,`receiver_email`, `created`, `edited`, `deleted`, `subject`, `body`, `group_id`) VALUES ('" . DBManager::RealEscape($this->Id) . "', '" . DBManager::RealEscape($this->MailboxId) . "', '" . DBManager::RealEscape($this->Email) . "', '" . DBManager::RealEscape($this->Name) . "', '" . DBManager::RealEscape($this->ReplyTo) . "','" . DBManager::RealEscape($this->ReceiverEmail) . "', '" . DBManager::RealEscape($this->Created) . "', '" . DBManager::RealEscape($time) . "', '" . DBManager::RealEscape($this->Deleted ? 1 : 0) . "', '" . DBManager::RealEscape($this->Subject) . "', '" . DBManager::RealEscape($this->Body) . "', '" . DBManager::RealEscape($this->GroupId) . "');");
        }
        setDataUpdateTime(DATA_UPDATE_KEY_EMAILS);
    }
...
class Ticket extends Action
{
    ...
    function PrepareEditorReply($_text)
    {
        global $CONFIG;
    ...
    function SendAutoresponder($_mailbox, $_message, $_pdm, $_att=null, $_subject="")
    {
        ...
        if($_mailbox != null)
        {
            ...
            if(!empty($CONFIG["gl_usmasend"]) && isValidEmail($this->Messages[0]->Email))
            {
                $mailbox->Email = $this->Messages[0]->Email;
                $mailbox->SenderName = $this->Messages[0]->Fullname;
            }

            if(!empty($CONFIG["gl_scom"]))
                sendMail($mailbox,$CONFIG["gl_scom"],$replytoint,$_message,getSubject($_subject,$this->Messages[0]->Email,$this->Messages[0]->Fullname,$this->Group,"",$this->Messages[0]->Company,$this->Messages[0]->Phone,$this->Messages[0]->IP,$this->Messages[0]->Text,$GROUPS[$this->Group]->GetDescription($this->Language),$this->Messages[0]->Customs),$_att);
            if(!empty($CONFIG["gl_sgom"]))
                sendMail($mailbox,$GROUPS[$this->Group]->Email,$replytoint,$_message,getSubject($_subject,$this->Messages[0]->Email,$this->Messages[0]->Fullname,$this->Group,"",$this->Messages[0]->Company,$this->Messages[0]->Phone,$this->Messages[0]->IP,$this->Messages[0]->Text,$GROUPS[$this->Group]->GetDescription($this->Language),$this->Messages[0]->Customs),$_att);
            if(!empty($CONFIG["gl_ssom"]) && isValidEmail($this->Messages[0]->Email))
                sendMail($mailbox,str_replace(";",",",$this->Messages[0]->Email),$replytoex,$_message,getSubject($_subject,$this->Messages[0]->Email,$this->Messages[0]->Fullname,$this->Group,"",$this->Messages[0]->Company,$this->Messages[0]->Phone,$this->Messages[0]->IP,$this->Messages[0]->Text,$GROUPS[$this->Group]->GetDescription($this->Language),$this->Messages[0]->Customs),$_att);
        }
    }
...
class ChatAutoReply
{
	...
	function GetXML()
	{
		return "<bf i=\"".base64_encode($this->Id)."\" l=\"".base64_encode($this->Languages)."\" n=\"".base64_encode($this->NewWindow ? 1 : 0)."\" ds=\"".base64_encode($this->Send ? 1 : 0)."\" w=\"".base64_encode($this->Waiting ? 1 : 0)."\" r=\"".base64_encode($this->ResourceId)."\" s=\"".base64_encode($this->SearchType)."\" a=\"".base64_encode($this->Answer)."\">".base64_encode($this->Tags)."</bf>\r\n";
	}

	function Save($_botId)
	{
		queryDB(true,"INSERT INTO `".DB_PREFIX.DATABASE_AUTO_REPLIES."` (`id` ,`resource_id` ,`owner_id` ,`tags` ,`search_type`,`answer`,`new_window`,`language`,`send`,`waiting`) VALUES ('".DBManager::RealEscape($this->Id)."','".DBManager::RealEscape($this->ResourceId)."','".DBManager::RealEscape($_botId)."','".DBManager::RealEscape($this->Tags)."','".DBManager::RealEscape($this->SearchType)."','".DBManager::RealEscape($this->Answer)."','".DBManager::RealEscape($this->NewWindow ? 1 : 0)."','".DBManager::RealEscape($this->Languages)."','".DBManager::RealEscape($this->Send ? 1 : 0)."','".DBManager::RealEscape($this->Waiting ? 1 : 0)."');");
	}
