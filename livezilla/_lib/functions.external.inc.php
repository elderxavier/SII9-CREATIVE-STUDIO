...
function login()
{
	...
	if(empty($_POST[POST_EXTERN_USER_QUESTION]) && !isnull(getCookieValue("form_114")) && $INPUTS[114]->Cookie)
		...
		$USER->Browsers[0]->Question = cutString(base64UrlDecode($_POST[POST_EXTERN_USER_QUESTION]),MAX_INPUT_LENGTH);
...
function getOperatorParameter($opParam="")
{
    if(!empty($_POST[POST_EXTERN_REQUESTED_INTERNID]))
        $opParam = base64UrlDecode($_POST[POST_EXTERN_REQUESTED_INTERNID]);
    else if(!empty($_GET[GET_EXTERN_INTERN_USER_ID]))
        $opParam = base64UrlDecode($_GET[GET_EXTERN_INTERN_USER_ID]);
    return $opParam;
}
...
function setOperator($util=0,$fromCookie=null,$result=true,$_allowBots=false,$_requireBot=false)
{
	...
	{
		if(!$internal->Deactivated && !$internal->MobileSleep($USER->Browsers[0]->Forward) && $internal->Status != USER_STATUS_OFFLINE && ($_allowBots || !$internal->IsBot) && (!$_requireBot || $internal->IsBot))
		{
			...
			if(in_array($USER->Browsers[0]->DesiredChatGroup,$internal->GetGroupList(true)))
			{
				...
				if(!$internal->IsBot && $intstatus < USER_STATUS_OFFLINE && $GROUPS[$USER->Browsers[0]->DesiredChatGroup]->MaxChatAmount > -1 && $GROUPS[$USER->Browsers[0]->DesiredChatGroup]->MaxChatAmount <= $group_chats[$sessId])
					$intstatus = $GROUPS[$USER->Browsers[0]->DesiredChatGroup]->MaxChatsStatus;
