...
function loadInternals()
{
    ...
    if(DB_CONNECTION)
    {
        ...
        $result = queryDB(false,"SELECT * FROM `".DB_PREFIX.DATABASE_AUTO_REPLIES."` ORDER BY `search_type` DESC, `tags` ASC;");
        if($result)
            while($row = DBManager::FetchArray($result))
...
function loadVisitors($_fullList=false,$_sqlwhere="",$_limit="",$count=0)
{
    ...
    if($result)
    {
        ...
        if($result)
            while($row = DBManager::FetchArray($result))
                if(isset($tvisitors[$row["visitor_id"]]) && $tvisitors[$row["visitor_id"]]->VisitId == $row["visit_id"])
                {
...
function initData($_fields)
{
    ...
    if(in_array("INPUTS",$_fields) && empty($INPUTS))loadInputs();
    if(in_array("FILTERS",$_fields) && empty($FILTERS))loadFilters();
    if(is("DB_CONNECTION"))
    {
        if(in_array("EVENTS",$_fields) && empty($EVENTS))loadEvents();
        if(in_array("VISITOR",$_fields) && empty($VISITOR))loadVisitors();
    }
}
...
function setCookieValue($_key,$_value,$_onlyWhenEmpty=false)
{
	...
	{
        if($_onlyWhenEmpty && getCookieValue($_key) != null)
            return;
...
function cronJobs($_asCronJob)
{
    ...
    if(rand(1,$randoms[0]) == 1)
	{
        ...
        queryDB(true,"DELETE FROM `".DB_PREFIX.DATABASE_VISITOR_BROWSERS."` WHERE `created` < ".DBManager::RealEscape(time()-$timeouts[3]));
        ...
        queryDB(true,"DELETE FROM `".DB_PREFIX.DATABASE_VISITOR_BROWSER_URLS."` WHERE `entrance` < ".DBManager::RealEscape(time()-$timeouts[3]));
...
function closeArchiveEntry($_chatId,$_externalFullname,$_externalId,$_internalId,$_groupId,$_email,$_company,$_phone,$_host,$_ip,$_question,$_transcriptSent=false,$_waitingtime,$_startResult,$_endResult)
{
    ...
    setDataUpdateTime(DATA_UPDATE_KEY_CHATS);
	...
	if($result)
		while($row = DBManager::FetchArray($result))
		{
			...
			if(!empty($mailbox) && !empty($CONFIG["gl_scct"]))
				sendMail($mailbox,$CONFIG["gl_scct"],$mailbox->Email,$rcvs,$subject);
...
{
	$posts = array();
	if($result = queryDB(true,"SELECT * FROM `".DB_PREFIX.DATABASE_POSTS."` WHERE `receiver`='".DBManager::RealEscape($_receiver)."' AND `received`=0 ORDER BY `time` ASC, `micro` ASC;"))
		while($row = DBManager::FetchArray($result))
			$posts[] = $row;
	return $posts;
}
...
function setDataUpdateTime($_areaIndex,$_reload=false)
{
    global $DUTU;
    ...
    if(!(isset($DUTU[$_areaIndex]) && $DUTU[$_areaIndex]==$mt))
    {
        ...
        if(DBManager::GetRowCount($result) == 0 && !$_reload)
        {
            ...
            setDataUpdateTime($_areaIndex,true);
        }
        else
