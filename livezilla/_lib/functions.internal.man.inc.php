        ...
        if(isset($INTERNAL[CALLER_SYSTEM_ID]) && ($INTERNAL[CALLER_SYSTEM_ID]->Level == USER_LEVEL_ADMIN || (is_array($INTERNAL[CALLER_SYSTEM_ID]->WebsitesUsers) && in_array($CONFIG["gl_host"],$INTERNAL[CALLER_SYSTEM_ID]->WebsitesUsers))))
        {
            ...
            while(isset($_POST["p_operators_" . $count . "_id"]))
            {
                ...
                else
                {
                    ...
                    queryDB(true,"REPLACE INTO `".$_prefix.DATABASE_OPERATORS."` (`id`, `system_id`, `fullname`, `description`, `email`, `permissions`, `webspace`, `password`, `status`, `level`, `visitor_file_sizes`, `groups`, `groups_status`, `groups_hidden`,`reposts`, `languages`, `auto_accept_chats`, `login_ip_range`, `websites_users`, `websites_config`, `bot`, `wm`, `wmohca`,`first_active`,`last_active`,`sign_off`,`lweb`,`lapp`,`mobile_os`,`mobile_device_id`,`mobile_background`,`mobile_ex`) VALUES ('".DBManager::RealEscape($_POST["p_operators_" . $count . "_id"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_system_id"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_fullname"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_description"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_email"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_permissions"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_webspace"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_password"])."','".$sta."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_level"])."','','".DBManager::RealEscape($_POST["p_operators_" . $count . "_groups"])."','','".DBManager::RealEscape($_POST["p_operators_" . $count . "_groups_hidden"])."','','".DBManager::RealEscape($_POST["p_operators_" . $count . "_languages"])."',0,'".DBManager::RealEscape($_POST["p_operators_" . $count . "_lipr"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_websites_users"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_websites_config"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_bot"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_wm"])."','".DBManager::RealEscape($_POST["p_operators_" . $count . "_wmohca"])."',".$fac.",".$lac.",".((empty($_POST["p_operators_" . $count . "_deac"])) ? 0 : 2).",'".DBManager::RealEscape($wcl ? 1:0)."','".DBManager::RealEscape($acl ? 1:0)."','".DBManager::RealEscape($aos)."','".DBManager::RealEscape($did)."','".DBManager::RealEscape($abm ? 1:0)."','".DBManager::RealEscape(@$_POST["p_operators_" . $count . "_mobile_ex"])."');");
                }
...
function testDataBase($_host,$_user,$_pass,$_dbname,$_prefix,$_extension="",$_intense=false)
{
	...
	else
	{
		...
		else
		{
			...
			$rowmci = @DBManager::FetchArray($connection->Query(false,"SELECT MAX(`chat_id`) as `mcid` FROM `".DBManager::RealEscape($_prefix).DATABASE_CHAT_ARCHIVE."`"));
			$rowmti = @DBManager::FetchArray($connection->Query(false,"SELECT MAX(`id`) as `mtid` FROM `".DBManager::RealEscape($_prefix).DATABASE_TICKETS."`"));
			
			if(!empty($rowmci["mcid"]) && is_numeric($rowmci["mcid"]) && $rowmci["mcid"] > $row["chat_id"])
                $connection->Query(false,"UPDATE `".DBManager::RealEscape($_prefix).DATABASE_INFO."` SET `chat_id`=".DBManager::RealEscape($rowmci["mcid"]));
			
			if(!empty($rowmti["mtid"]) && is_numeric($rowmti["mtid"]) && $rowmti["mtid"] > $row["ticket_id"])
                $connection->Query(false,"UPDATE `".DBManager::RealEscape($_prefix).DATABASE_INFO."` SET `ticket_id`=".DBManager::RealEscape($rowmti["mtid"]));
