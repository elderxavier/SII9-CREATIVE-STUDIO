...
ChatDisplayLayoutClass.prototype.resizeOptions = function() {
    if ($('#user-settings-dialog-body').length > 0) {
        ...
        var chatSettingsHeight = tabContentHeight - 22, backgroundSettingsHeight = 0;
        if (lzm_chatDisplay.isApp && appOs == 'android') {
            chatSettingsHeight = Math.max(35, Math.floor(tabContentHeight / 2) - 22);
            backgroundSettingsHeight = Math.max(35, Math.floor(tabContentHeight / 2) - 27);
        }
...
ChatDisplayLayoutClass.prototype.resizeAddResources = function() {
    if ($('#qrd-add-body').length > 0 || $('#qrd-tree-dialog-body').length > 0 || $('#ticket-details-body').length > 0) {
        var qrdTextHeight = Math.max((lzm_chatDisplay.FullscreenDialogWindowHeight - 307), 100);
        var textWidth = lzm_chatDisplay.FullscreenDialogWindowWidth - 50;
...
ChatDisplayLayoutClass.prototype.resizeEditResources = function() {
    if ($('#qrd-edit-body').length > 0) {
        var qrdTextHeight = Math.max((lzm_chatDisplay.FullscreenDialogWindowHeight - 251), 100);
        var textWidth = lzm_chatDisplay.FullscreenDialogWindowWidth - 50;
