...
ChatPollServerClass.prototype.startPolling = function (noFirstPoll) {
    //console.log('Polling started: ' + lzm_chatTimeStamp.getServerTimeString());
...
ChatPollServerClass.prototype.pollServerTicket = function(ticket, emails, type, chat) {
    ...
    if (!thisClass.pollIsActive) {
        ...
        if (type == 'save-details') {
            ...
            thisClass.ticketUpdateTimestamp = 0;
            thisClass.dataObject.p_gl_a = 'N';
            ...
            if (typeof ticket.cf != 'undefined') {
                for (var key in ticket.cf) {
                    if (ticket.cf.hasOwnProperty(key) && parseInt(key) < 111) {
                        ticketDataObject['p_ta_' + count + '_vd_' + (16 + parseInt(key))] = ticket.cf[key];
                    }
                }
            }
            if (chat.cid != '') {
        ...
        if (emails.length > 0) {
            ...
            for (var j=0; j<emails[1].length; j++) {
                ...
                for (var k=0; k<emails[1][j].attachment.length; k++) {
                    ...
                    //console.log(emails[1][j].attachment[k]);
                    vdCount++;
                }
                ...
                for (var key in emails[1][j].custom) {
                    if (emails[1][j].custom.hasOwnProperty(key) && parseInt(key) < 111) {
                        ticketDataObject['p_ta_' + count + '_vd_' + (16 + parseInt(key))] = emails[1][j].custom[key];
                    }
                }
                count++;
            ...
            thisClass.emailUpdateTimestamp = 0;
            thisClass.dataObject.p_gl_a = 'N';
            //console.log('Resetting email uopdate timestamp');
        }

        //console.log(ticketDataObject);
    ...
    if (!thisClass.pollIsActive) {
        ...
        try {
            ...
            this.fileUploadClient.onabort = function(e) {
                ...
                try {
                    console.log('Uploading file aborted');
                    console.log(e);
                } catch (e) {}
            };
...
ChatPollServerClass.prototype.pollServerlogin = function (serverProtocol, serverUrl, logoutOtherInstance) {
    ...
    /*if (debug) {
        loginDataObject.p_app_os = 'test';
        loginDataObject.p_app_device_id = 'LOGIN';
        loginDataObject.p_app_language = lzm_t.language;
        loginDataObject.p_app_background = 0;
        loginDataObject.p_app = 1;
    }*/
    ...
    //console.log(JSON.stringify(thisClass.chosenProfile.login_name));
...
ChatPollServerClass.prototype.fillDataObject = function () {
    ...
    /*if (debug) {
        this.dataObject.p_app_os = 'test';
        this.dataObject.p_app_language = lzm_t.language;
        this.dataObject.p_app_device_id = '1111111111111111222222222222222233333333333333334444444444444444';
        this.dataObject.p_app_background = debugBackgroundMode ? 1 : 0;
        this.dataObject.p_app = 1;
    }*/
...
ChatPollServerClass.prototype.evaluateServerResponse = function (xmlString, type) {
    ...
    if (xmlString != '') {
        ...
        /*if (xmlString.match(/VEVTVA==/) != null) {
            logit(xmlString);
        }*/
    }
    ...
    if (thisClass.lzm_chatServerEvaluation.login_data.timediff * 1000 != thisClass.lzm_chatServerEvaluation.lzm_chatTimeStamp.timeDifference) {
        //console.log(thisClass.lzm_chatServerEvaluation.login_data.timediff);
    ...
    try {
        if (xmlString != '') {
            ...
            if (xmlIsLiveZillaXml) {
                ...
                if ($.inArray(validationError, [-1, 1, 11]) == -1) {
                    //alert ('The validation error ' + validationError + ' was of type ' + typeof validationError);
                    //thisClass.logout();
                    thisClass.stopPolling();
                ...
                } else if ($.inArray(validationError, [1, 11]) != -1) {
                    try {
                        //console.log('Validation error ' + validationError + ' occured');
                    } catch(ex) {
                        // Do nothing...
                    }
                }
                ...
                //console.log(thisClass.ticketUpdateTimestamp);
                ...
                if (thisClass.lzm_chatServerEvaluation.myId != '') {
                    if (thisClass.qrdRequestTime == 0) {
                        ...
                        thisClass.lzm_chatDisplay.showViewSelectPanel = (showViewSelectPanel != null && showViewSelectPanel != '') ?
                            JSON.parse(showViewSelectPanel) : thisClass.lzm_chatDisplay.showViewSelectPanel;
                        thisClass.lzm_chatDisplay.viewSelectArray = (viewSelectArray != null && viewSelectArray != '') ?
                            JSON.parse(viewSelectArray) : thisClass.lzm_chatDisplay.viewSelectArray;
                    ...
                    if (thisClass.ticketMaxRead == 0) {
                        ...
                        //console.log (thisClass.ticketMaxRead);
                    }
                }


                if (thisClass.lzm_chatServerEvaluation.new_qrd) {
                    thisClass.lzm_chatDisplay.updateResources(thisClass.lzm_chatServerEvaluation.resources);
                }

                // depending on the server response recreate parts of the html
                if ((thisClass.lzm_chatServerEvaluation.new_dt || thisClass.lzm_chatServerEvaluation.new_de) && !thisClass.lzm_chatServerEvaluation.ticketGlobalValues['no_update']) {
                    //console.log('Updating ticket list');
                    //console.log(thisClass.lzm_chatServerEvaluation.ticketGlobalValues.e);
                    thisClass.lzm_chatDisplay.updateTicketList(thisClass.lzm_chatServerEvaluation.tickets,
                        ...
                        thisClass.lzm_chatServerEvaluation.internal_users);
                }
                if (thisClass.lzm_chatServerEvaluation.new_de && $('#email-list-container').length > 0) {
                    thisClass.lzm_chatDisplay.updateEmailList(thisClass.lzm_chatServerEvaluation.emails, thisClass.lzm_chatServerEvaluation.internal_departments);
                }
                ...
                if (thisClass.lzm_chatServerEvaluation.new_ext_u) {
                    ...
                    if (userUpdated) {
                        ...
                        thisClass.lzm_chatDisplay.updateVisitorInformation(thisClass.lzm_chatServerEvaluation.internal_users,
                            thisClass.lzm_chatDisplay.infoUser);
                    }
                }
            ...
            if (thisClass.ticketLimit != lzm_chatServerEvaluation.ticketGlobalValues.p) {
                thisClass.ticketUpdateTimestamp = 0;
                thisClass.addPropertyToDataObject('p_gl_a', 'N');
            }
            if (thisClass.chatArchiveLimit != lzm_chatServerEvaluation.chatArchive.p) {
                thisClass.chatUpdateTimestamp = 0;
                thisClass.addPropertyToDataObject('p_gl_a', 'N');
            }
        } else {
            thisClass.lastCorrectServerAnswer = lzm_chatTimeStamp.getServerTimeString(null, false, 1);
        ...
        if (lzm_chatTimeStamp.getServerTimeString(null, false, 1) - thisClass.lastCorrectServerAnswer >= thisClass.maxTimeSinceLastCorrectAnswer) {
            thisClass.stopPolling();
            //console.log('Error count ' + thisClass.errorCount + ' - log out');
            thisClass.finishLogout('parseError');
        } else {
            thisClass.errorCount++;
            thisClass.pollIsActive = false;
...
ChatPollServerClass.prototype.finishLogout = function(cause, jqXHR, postUrl) {
    //console.log('Finishing logout because of ' + cause);
    //console.log(jqXHR);
    var thisClass = this;
    ...
    if (this.isApp == 1) {
        try {
            ...
            console.log('Opening login view failed.');
        }
    } else {
