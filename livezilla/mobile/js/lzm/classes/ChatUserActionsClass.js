...
ChatUserActionsClass.prototype.inviteExternalUser = function (id, b_id, text) {
    ...
    for (var i=0; i<this.lzm_chatServerEvaluation.external_users.length; i++) {
        if (this.lzm_chatServerEvaluation.external_users[i].id == id) {
            this.lzm_chatServerEvaluation.external_users[i].invitation = {tbid: b_id, status: 'requested', req: this.lzm_chatServerEvaluation.myId, logo: 'img/632-skills.png'};
            this.lzm_chatServerEvaluation.external_users[i].tbid = b_id;
            this.lzm_chatServerEvaluation.external_users[i].req = this.lzm_chatServerEvaluation.myId;
            this.lzm_chatServerEvaluation.changedExternalUsers.push(id);
            break;
        }
    }
    this.lzm_chatDisplay.updateVisitorList(this.lzm_chatServerEvaluation.external_users,this.lzm_chatServerEvaluation.chatObject,this.lzm_chatServerEvaluation.internal_users);
};
...
ChatUserActionsClass.prototype.getChatPM = function(visitorId, browserId, pmId, language, groupId, userId) {
    ...
    //console.info(visitorId + ' ~ ' + browserId);
    for (i=0; i<this.lzm_chatServerEvaluation.external_users.length; i++) {
        if (visitorId == this.lzm_chatServerEvaluation.external_users[i].id) {
            for (j=0; j<this.lzm_chatServerEvaluation.external_users[i].b.length; j++) {
                if (browserId == this.lzm_chatServerEvaluation.external_users[i].b[j].id) {
                    ...
                    if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].cname != 'undefined') {
                        visitorName = this.lzm_commonTools.htmlEntities(this.lzm_chatServerEvaluation.external_users[i].b[j].cname);
                    }
                    if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].cemail != 'undefined') {
                        visitorEmail = this.lzm_commonTools.htmlEntities(this.lzm_chatServerEvaluation.external_users[i].b[j].cemail);
                    }
                    if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].ccompany != 'undefined') {
                        visitorCompany = this.lzm_commonTools.htmlEntities(this.lzm_chatServerEvaluation.external_users[i].b[j].ccompany);
                    }
                    if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].cphone != 'undefined') {
                        visitorPhone = this.lzm_commonTools.htmlEntities(this.lzm_chatServerEvaluation.external_users[i].b[j].cphone);
                    }
                    if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].eq != 'undefined') {
                        visitorQuestion = this.lzm_commonTools.htmlEntities(this.lzm_chatServerEvaluation.external_users[i].b[j].eq);
                    }
                    if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].id != 'undefined') {
                        visitorChatId = this.lzm_chatServerEvaluation.external_users[i].b[j].id;
                    }
                    break;
                }
            }
            try {
                for (j=0; j<this.lzm_chatServerEvaluation.external_users[i].b.length; j++) {
                    if (browserId.indexOf('_OVL') == -1) {
                        var hLast = this.lzm_chatServerEvaluation.external_users[i].b[j].h2.length - 1;
                        if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].h2[hLast].url != 'undefined') {
                            visitorUrl = this.lzm_chatServerEvaluation.external_users[i].b[j].h2[hLast].url;
                        }
                        if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].h2[hLast].title != 'undefined') {
                            visitorPageTitle = this.lzm_chatServerEvaluation.external_users[i].b[j].h2[hLast].title;
                        }
                        if (typeof this.lzm_chatServerEvaluation.external_users[i].b[j].ss != 'undefined') {
                            visitorSearchString = this.lzm_chatServerEvaluation.external_users[i].b[j].ss;
                        }
                        break;
                    }
                }
            ...
            if (typeof this.lzm_chatServerEvaluation.external_users[i].ip != 'undefined') {
                visitorIp = this.lzm_chatServerEvaluation.external_users[i].ip;
            }
            break;
        }
    }
    ...
    //console.log(pmLanguages);
    //var globalDefaultLanguage = this.lzm_chatServerEvaluation.defaultLanguage.toUpperCase();
    ...
    if (!groupIdWasGiven) {
        ...
        for (i=0; i<this.lzm_chatServerEvaluation.internal_users.length; i++) {
            if (userId == this.lzm_chatServerEvaluation.internal_users[i].id &&
                ...
                for (j=0; j<this.lzm_chatServerEvaluation.internal_users[i].pm.length; j++) {
                    if (chatLang == this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang) {
                        userPm = this.lzm_commonTools.clone(this.lzm_chatServerEvaluation.internal_users[i].pm[j]);
                    }
                    if (chatLangShort == this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang) {
                        userPm2 = this.lzm_commonTools.clone(this.lzm_chatServerEvaluation.internal_users[i].pm[j]);
                    }
                    if (globalDefaultLanguage == this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang) {
                        userFallbackPm = this.lzm_commonTools.clone(this.lzm_chatServerEvaluation.internal_users[i].pm[j]);
                    }
                }
                break;
            }
    ...
    if (!userIdWasGiven) {
        ...
        //console.log(groupId);
        for (i=0; i<this.lzm_chatServerEvaluation.internal_departments.length; i++) {
            if (groupId == '' || groupId == this.lzm_chatServerEvaluation.internal_departments[i].id) {
                for (j=0; j<this.lzm_chatServerEvaluation.internal_departments[i].pm.length; j++) {
                    if (chatLang == this.lzm_chatServerEvaluation.internal_departments[i].pm[j].lang) {
                        pm = this.lzm_commonTools.clone(this.lzm_chatServerEvaluation.internal_departments[i].pm[j]);
                    }
                    if (chatLangShort == this.lzm_chatServerEvaluation.internal_departments[i].pm[j].lang) {
                        pm2 = this.lzm_commonTools.clone(this.lzm_chatServerEvaluation.internal_departments[i].pm[j]);
                    }
                    if (globalDefaultLanguage == this.lzm_chatServerEvaluation.internal_departments[i].pm[j].lang) {
                        fallbackPm = this.lzm_commonTools.clone(this.lzm_chatServerEvaluation.internal_departments[i].pm[j]);
                    }
                }
                break;
            }
...
ChatUserActionsClass.prototype.getPmLanguages = function(groupId) {
    ...
    var i, j;
    for (i=0; i<this.lzm_chatServerEvaluation.internal_departments.length; i++) {
        if (this.lzm_chatServerEvaluation.internal_departments[i].id == groupId || groupId == '') {
            for (j=0; j<this.lzm_chatServerEvaluation.internal_departments[i].pm.length; j++) {
                pmLanguages.group.push(this.lzm_chatServerEvaluation.internal_departments[i].pm[j].lang);
                pmLanguages.all.push(this.lzm_chatServerEvaluation.internal_departments[i].pm[j].lang);
                if (this.lzm_chatServerEvaluation.internal_departments[i].pm[j].def == '1') {
                    pmLanguages.default = ['group', this.lzm_chatServerEvaluation.internal_departments[i].pm[j].lang];
                }
            }
            break;
        }
    ...
    for (i=0; i<this.lzm_chatServerEvaluation.internal_users.length; i++) {
        if (this.lzm_chatServerEvaluation.internal_users[i].id == this.lzm_chatServerEvaluation.myId) {
            try {
                for (j=0; j<this.lzm_chatServerEvaluation.internal_users[i].pm.length; j++) {
                    if ($.inArray(this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang, pmLanguages) == -1) {
                        pmLanguages.user.push(this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang);
                        pmLanguages.all.push(this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang);
                        if (this.lzm_chatServerEvaluation.internal_users[i].pm[j].def == '1') {
                            pmLanguages.default = ['user', this.lzm_chatServerEvaluation.internal_users[i].pm[j].lang];
                        }
                    }
                }
            } catch(ex) {}
...
ChatUserActionsClass.prototype.viewUserData = function (id, b_id, chat_id, freeToChat, newlyAcceptedChat) {
    //console.log('View user data: ' + id + ', ' + b_id + ', ' + chat_id);
    var thisClass = this;
    ...
    for (var i = 0; i < thisClass.lzm_chatServerEvaluation.external_users.length; i++) {
        if (thisClass.lzm_chatServerEvaluation.external_users[i].id == id) {
            ...
            active_chat = thisClass.lzm_chatServerEvaluation.external_users[i].id;
            visitorLangString = thisClass.lzm_chatServerEvaluation.external_users[i].lang;
            ...
            if (thisClass.lzm_chatServerEvaluation.external_users[i].b_id != b_id) {
                for (var j=0; j<thisClass.lzm_chatServerEvaluation.external_users[i].b.length; j++) {
                    if (thisClass.lzm_chatServerEvaluation.external_users[i].b[j].id == b_id) {
                        ...
                        active_chat_reco = thisClass.lzm_chatServerEvaluation.external_users[i].id + '~'
                            + thisClass.lzm_chatServerEvaluation.external_users[i].b[j].id;
                        if (typeof thisClass.lzm_chatServerEvaluation.external_users[i].b[j].cname != 'undefined' &&
                            thisClass.lzm_chatServerEvaluation.external_users[i].b[j].cname != '') {
                            active_chat_realname = thisClass.lzm_chatServerEvaluation.external_users[i].b[j].cname;
                        } else {
                            active_chat_realname = thisClass.lzm_chatServerEvaluation.external_users[i].id;
                        }
                        break;
                    }
                }
    ...
    if ($.inArray(id + '~' + b_id, thisClass.open_chats) != -1) {
        ...
        removeEditor();
        //console.log('Not an open chat - removing editor');
...
ChatUserActionsClass.prototype.acceptChat = function(id, b_id, chat_id, active_chat_reco, visitorLangString) {
    var thisClass = this;
    //console.log('Accept chat from external user ' + id + '~' + b_id);
    if ($.inArray(id + '~' + b_id, thisClass.open_chats) == -1) {
...
ChatUserActionsClass.prototype.chatExternalWith = function (id, b_id, chat_id, chat_no) {
    ...
    if (thisClass.lzm_chatServerEvaluation.chatObject[thisClass.lzm_chatServerEvaluation.active_chat_reco]['status'] == 'new') {
        //console.log ('Set chat status to read');
        thisClass.lzm_chatServerEvaluation.chatObject[thisClass.lzm_chatServerEvaluation.active_chat_reco]['status'] = 'read';
    }
    //console.log('Chat status: ' + thisClass.lzm_chatServerEvaluation.chatObject[thisClass.lzm_chatServerEvaluation.active_chat_reco]['status']);
...
ChatUserActionsClass.prototype.forwardChat = function () {
    if (typeof this.forwardData.id != 'undefined') {
        ...
        var extUserName = '';
        for (var extUserIndex=0; extUserIndex<this.lzm_chatServerEvaluation.external_users.length; extUserIndex++) {
            if (this.lzm_chatServerEvaluation.external_users[extUserIndex].id == this.forwardData.id) {
                for (var browserIndex=0; browserIndex<this.lzm_chatServerEvaluation.external_users[extUserIndex].b.length; browserIndex++) {
                    if (this.lzm_chatServerEvaluation.external_users[extUserIndex].b[browserIndex].id == this.forwardData.b_id) {
                        extUserName = (this.lzm_chatServerEvaluation.external_users[extUserIndex].b[browserIndex].cname != '') ?
                            this.lzm_chatServerEvaluation.external_users[extUserIndex].b[browserIndex].cname :
                            this.lzm_chatServerEvaluation.external_users[extUserIndex].unique_name;
                        break;
                    }
                }
                break;
...
ChatUserActionsClass.prototype.saveUserSettings = function(settings, multiServerId, isApp) {
    ...
    //console.log(JSON.stringify(this.chosenProfile));
    ...
    //alert(settings.playNewTicketSound);
...
ChatUserActionsClass.prototype.previewQrd = function (chatPartner, qrdId, inDialog, menuEntry) {
    ...
    for (var i=0; i<thisClass.lzm_chatServerEvaluation.resources.length; i++) {
        if (thisClass.lzm_chatServerEvaluation.resources[i].rid == qrdId) {
            resource = thisClass.lzm_chatServerEvaluation.resources[i];
            break;
        }
    }
    var thisChatPartner = lzm_displayHelper.getChatPartner(chatPartner, thisClass.lzm_chatServerEvaluation.external_users,
        thisClass.lzm_chatServerEvaluation.internal_users, thisClass.lzm_chatServerEvaluation.internal_departments);
...
ChatUserActionsClass.prototype.addQrd = function(ticketId, inDialog, toAttachment, menuEntry) {
    ...
    $('#save-new-qrd').click(function() {
        ...
        if (newType != 3) {
            ...
            lzm_chatServerEvaluation.resources.push(newResource);
            lzm_chatServerEvaluation.resourceIdList.push(newRid);
        ...
        lzm_chatDisplay.updateResources(lzm_chatServerEvaluation.resources);
...
ChatUserActionsClass.prototype.saveTicketDetails = function(ticket, channel, status, group, editor, language, name,
                                        email, company, phone, message, attachments, comments, customFields, chat) {
    ...
    if (typeof ticket.id != 'undefined') {
        ...
        this.lzm_chatPollServer.pollServerTicket({
            id: id, ne: editor, ns: status, ng: group, oe: oe, os: os, og: og, nl: language, ol: ol
        }, [], 'save-details');
    } else {
        this.lzm_chatPollServer.pollServerTicket({
