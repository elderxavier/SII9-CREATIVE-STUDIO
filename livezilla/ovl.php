...
{
	$changed=false;
	$USER->Browsers[0]->Fullname = $USER->Browsers[1]->Fullname = getOParam("en",$USER->Browsers[0]->Fullname,$changed);
	$USER->Browsers[0]->Email = $USER->Browsers[1]->Email = getOParam("ee",$USER->Browsers[0]->Email,$changed);
	$USER->Browsers[0]->Company = getOParam("ec",$USER->Browsers[0]->Company,$changed);
	$USER->Browsers[0]->Code = getOParam("code",$USER->Browsers[0]->Code,$changed);
	$USER->Browsers[0]->Phone = getOParam("ep",$USER->Browsers[0]->Phone,$changed);
	$USER->Browsers[0]->Question = getOParam("eq",$USER->Browsers[0]->Question,$changed);
    ...
    {
	    $TRACKINGSCRIPT .= "lz_tracking_add_overlay_chat('".base64_encode($chat)."','".base64_encode(base64UrlDecode($text))."',280,378,".getOParam("ovlml",0,$nu,FILTER_SANITIZE_NUMBER_INT).",".getOParam("ovlmt",0,$nu,FILTER_SANITIZE_NUMBER_INT).",".getOParam("ovlmr",0,$nu,FILTER_SANITIZE_NUMBER_INT).",".getOParam("ovlmb",0,$nu,FILTER_SANITIZE_NUMBER_INT).",'".getOParam("ovlp",21,$nu,FILTER_SANITIZE_NUMBER_INT)."',true,".parseBool($conline).");";
    ...
    $USER->AddFunctionCall("lz_chat_set_application(".parseBool($chat_available).",".parseBool($BOTMODE/* && !$HUMAN && !(!empty($INTERNAL[$USER->Browsers[0]->DesiredChatPartner]) && !$INTERNAL[$USER->Browsers[0]->DesiredChatPartner]->IsBot && $INTERNAL[$USER->Browsers[0]->DesiredChatPartner]->IsAvailableForChat())*/).",".parseBool($HUMAN).",'".$bottitle."');",false);
    ...
    if(!$openChatExternal && !empty($USER->Browsers[1]->ChatRequest) && $INTERNAL[$USER->Browsers[1]->ChatRequest->SenderSystemId]->IsExternal($GROUPS,null,null,true))
    {
		...
		if($all > 0)
		{
			...
			while($row = DBManager::FetchArray($result))
			{
				if($count++ >= 0)
				{
					...
					$post = postHTML($postobj->Text,$postobj->Translation,($lppflag != $postobj->Sender || $pstrchngreq),$postobj->Sender != $USER->Browsers[0]->SystemId,(($postobj->Sender != $USER->Browsers[0]->SystemId) ? $postobj->SenderName : htmlentities($USER->Browsers[0]->Fullname,ENT_QUOTES,"UTF-8")));
